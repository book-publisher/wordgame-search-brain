...
async function generatePDF(puzzlesData, trimSizeStr, solutionsPerPage) {
    ...
    // ── Draw mini solution grid for multi-solution pages ────────────
    function drawMiniSolution(puzzleData, puzzleNum, ox, oy, boxW, boxH) {
        ...
        pdf.setFontSize(9);
        pdf.setTextColor(0, 0, 0);
        pdf.text(`Puzzle #${puzzleNum}`, ox + boxW / 2, oy + 0.15, { align: 'center' });
    ...
    if (solutionsPerPage === 1) {
        ...
        const boxH = (usableH - 0.4) / solRows; // leave room for page header
        ...
        while (idx < puzzlesData.length) {
            ...
            pdf.setFontSize(14);
            pdf.setTextColor(0, 0, 0);
            pdf.text('Solutions', W / 2, MARGIN + 0.2, { align: 'center' });

            const startY = MARGIN + 0.4;
