...
function renderPuzzleToDOM(puzzleData, puzzleNum, isSolution = false, isSmallMode = false) {
    ...
    grid.className = `word-grid ${s.showBorder ? 'show-border' : ''}`;
    grid.style.position = 'relative';
    grid.style.fontFamily = `"${s.fontGrid}", monospace`;
    
    // Scale font size based on cols and whether it's small mode
    let baseFontSize = isSmallMode ? 11 : 18;
    if (s.cols > 15) baseFontSize -= 2;
    grid.style.fontSize = `${baseFontSize}px`;

    const cellW = isSmallMode ? 14 : (s.cols > 15 ? 22 : 30);
    
    // Use CSS Grid for precise alignment
    grid.style.gridTemplateColumns = `repeat(${s.cols}, ${cellW}px)`;
    grid.style.gridTemplateRows = `repeat(${s.rows}, ${cellW}px)`;
    ...
    // Highlight cells for Solutions (only highlight placed word letters)
    if (isSolution) {
